...
public class TableGroup extends ArrayList<Table> {
					...
					if (roll <= chance) {
						...
						if (chance <= 0.5) {
							if (policy.equals(TablePolicy.VERY_RARE) || policy.equals(TablePolicy.RARE)) {
								World.getWorld().sendWorldMessage("<img=8><col=FF0000>[Lootations] </col><col=255>" + player.getName() + "</col> received <col=255>" + item.getAmount() + "</col>x <col=255>" + ItemDefinition.forId(item.getId()) + "</col>.", false);
							}
						}
					}
				}
	...
	/**
	 * The non-playable character identification values that have access
