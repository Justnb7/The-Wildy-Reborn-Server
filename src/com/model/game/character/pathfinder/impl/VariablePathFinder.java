...
public class VariablePathFinder implements PathFinder {
	...
	public VariablePathFinder(int type, int walkToData, int direction,
			int sizeX, int sizeY) {
		this.type = type;
		...
		while (writePathPosition != readPosition) {
			...
			if (type != -1) {
				if (type == 0 || type == 1 || type == 2 || type == 3
						|| type == 9) {
					if (reachedObject(mob, dstX, dstY, absX, absY, curX, curY,
							type, direction)) {
						requestX = curX;
						requestY = curY;
						foundPath = true;
						state.setReached(true);
						break;
					}
				} else {
					if (reachedObject2(mob, dstX, dstY, absX, absY, curX, curY,
							type, direction)) {
						requestX = curX;
						requestY = curY;
						foundPath = true;
				...
				if (reachedObject3(mob, target, dstX, dstY, absX, absY, curX,
						curY, sizeX, sizeY, walkToData)) {
					requestX = curX;
					requestY = curY;
					foundPath = true;
					state.setReached(true);
			...
			if (curY > 0
					&& via[curX][curY - 1] == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX, absY - 1, z) & 0x1280102) == 0) {
				check(mob, curX, curY - 1, SOUTH_FLAG, thisCost);
			}
			if (curX > 0
					&& via[curX - 1][curY] == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX - 1, absY, z) & 0x1280108) == 0) {
				check(mob, curX - 1, curY, WEST_FLAG, thisCost);
			}
			if (curY < 103
					&& via[curX][curY + 1] == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX, absY + 1, z) & 0x1280120) == 0) {
				check(mob, curX, curY + 1, NORTH_FLAG, thisCost);
			}
			if (curX < 103
					&& via[curX + 1][curY] == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX + 1, absY, z) & 0x1280180) == 0) {
				check(mob, curX + 1, curY, EAST_FLAG, thisCost);
			}
			if (curX > 0
					&& curY > 0
					&& via[curX - 1][curY - 1] == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX - 1, absY - 1, z) & 0x128010e) == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX - 1, absY, z) & 0x1280108) == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX, absY - 1, z) & 0x1280102) == 0) {
			...
			if (curX > 0
					&& curY < 103
					&& via[curX - 1][curY + 1] == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX - 1, absY + 1, z) & 0x1280138) == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX - 1, absY, z) & 0x1280108) == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX, absY + 1, z) & 0x1280120) == 0) {
			...
			if (curX < 103
					&& curY > 0
					&& via[curX + 1][curY - 1] == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX + 1, absY - 1, z) & 0x1280183) == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX + 1, absY, z) & 0x1280180) == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX, absY - 1, z) & 0x1280102) == 0) {
			...
			if (curX < 103
					&& curY < 103
					&& via[curX + 1][curY + 1] == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX + 1, absY + 1, z) & 0x12801e0) == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX + 1, absY, z) & 0x1280180) == 0
					&& (World.getWorld().getRegionManager().getClippingMask(absX, absY + 1, z) & 0x1280120) == 0) {
				...
				for (int x = dstX - range; x <= dstX + range; x++) {
					for (int y = dstY - range; y <= dstY + range; y++) {
						if (x >= 0 && y >= 0 && x < 104 && y < 104
							...
							if (distance < fullCost
									|| (distance == fullCost && (cost[x][y] < thisCost))) {
								fullCost = distance;
								thisCost = cost[x][y];
								curX = x;
								curY = y;
	...
	public boolean reachedObject(Entity mob, int dstX, int dstY, int absX, int absY, int curX, int curY, int type, int direction) {
		if (curX == dstX && curY == dstY)
			return true;
		//z = 0;
		int clipping = World.getWorld().getRegionManager().getClippingMask(absX, absY, z);
		if (type == 0)
			if (direction == 0) {
	...
	public boolean reachedObject2(Mob mob, int dstX, int dstY, int absX,
			int absY, int curX, int curY, int type, int direction) {
		if (curX == dstX && curY == dstY)
			return true;
		z = 0;
		int clipping = World.getWorld().getRegionManager().getClippingMask(absX, absY, z);
	...
	private boolean reachedObject3(Mob mob, Mob target, int dstX,
			int dstY, int absX, int absY, int curX, int curY, int sizeX,
			int sizeY, int walkToData) {
		...
		if (curX == dstX - 1 && curY >= dstY && curY <= maxY
				&& (clipping & 8) == 0 && (walkToData & 8) == 0) {
			return true;
		}
		if (curX == maxX + 1 && curY >= dstY && curY <= maxY
				&& (clipping & 0x80) == 0 && (walkToData & 2) == 0) {
			return true;
		}
		if (curY == dstY - 1 && curX >= dstX && curX <= maxX
				&& (clipping & 2) == 0 && (walkToData & 4) == 0
				|| curY == maxY + 1 && curX >= dstX && curX <= maxX
				&& (clipping & 0x20) == 0 && (walkToData & 1) == 0) {
			return true;
		}
		return false;
	}
