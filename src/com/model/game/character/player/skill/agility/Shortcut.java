...
public class Shortcut {
	...
	public static void agilityWalk(final Player player, final int walkAnimation, final int x, final int y) {
		...
		player.write(new SendConfig(173, 0));
	...
	private static void handleAgility(Player player, int x, int y, int levelReq, int anim, int walk, String message) {
		if (player.getSkills().getLevel(Skills.AGILITY) < levelReq) {
			player.write(new SendMessagePacket("You need " + levelReq + " agility to use this shortcut."));
			return;
		}
	...
	public static void processAgilityShortcut(Player player) {
		...
		case 4616:
			if (player.absX == 2595) {
				handleAgility(player, 2599, player.absY, 1, 3067, MOVE, "You pass through the agility shortcut.");
			} else if (player.absX == 2599) {
				handleAgility(player, 2595, player.absY, 1, 3067, MOVE, "You pass through the agility shortcut.");
			}
			break;
		case 11844:
			if (player.absX == 2936) {
