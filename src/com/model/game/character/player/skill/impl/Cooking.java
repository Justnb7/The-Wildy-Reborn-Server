...
public class Cooking extends SkillTask {
	...
	//TODO add cake making, pizzas
	
	public static boolean makeCake(Player player, int item1, int item2) {
		int cake_tin = 1887;
		int egg = 1944;
		...
		if (item1 == cake_tin || item2 == egg || item1 == egg || item2 == cake_tin || item1 == bucket_of_milk || item2 == cake_tin || item1 == cake_tin || item2 == bucket_of_milk || item1 == cake_tin || item2 == pot_of_flour || item1 == pot_of_flour || item2 == cake_tin) {
			if (player.getItems().contains(egg, bucket_of_milk, pot_of_flour, cake_tin)) {
				if (!player.getItems().contains(egg, bucket_of_milk, pot_of_flour, cake_tin)) {
					player.write(new SendMessagePacket("You do not have the required ingredients to make a cake."));
					player.write(new SendMessagePacket("In order to make cake you need, a egg, bucket of milk and a pot of flower."));
					return false;
				}
				if (player.getItems().getFreeSlots() < 1) {
					player.write(new SendMessagePacket("Not enough inventory space."));
					return false;
				}
				player.getItems().deleteItem(egg);
				player.getItems().deleteItem(bucket_of_milk);
				player.getItems().deleteItem(pot_of_flour);
				player.getItems().deleteItem(cake_tin);
				player.getItems().addItem(uncooked_cake, 1);
				return true;
			}
		}
		
		int chocolate_bar = 1973;
		int cooked_cake = 1891;
		int chocolate_cake = 1897;
		
		if (item1 == cooked_cake || item2 == chocolate_bar || item1 == chocolate_bar || item2 == cooked_cake) {
			if (player.getItems().getFreeSlots() < 1) {
				player.write(new SendMessagePacket("Not enough inventory space."));
				return false;
			} else if (!player.getItems().playerHasItem(chocolate_bar, cooked_cake)) {
				player.write(new SendMessagePacket("You do not have the required ingredients to make a cake."));
				return false;
			}
			player.getItems().deleteItem(chocolate_bar);
			player.getItems().deleteItem(cooked_cake);
			player.getItems().addItem(chocolate_cake, 1);
			return true;
		}
		return false;
	}
