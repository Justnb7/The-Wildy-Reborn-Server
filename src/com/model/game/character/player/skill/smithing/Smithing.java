...
public class Smithing {
	...
	private static final int[] BAR_REQS = { 1, 15, 20, 30, 40, 50, 70, 85 };
	private static final int[] ORE_1 = { 438, 440, -1, 440, 444, 447, 449, 451 };
	private static final int[] ORE_2 = { 436, -1, -1, -1, -1, -1, -1, -1 };
	private static final int[] SMELT_EXP = { 6, 13, -1, 18, 23, 30, 38, 50 };
	...
	private static int oreId2;
	private static int barId;

	public void sendSmelting(Player player) {
		...
		player.smeltInterface = true;
	}

	public static void startSmelting(Player player, int barType) {
		if (canSmelt(player, barType)) {
			// player.sendMessage("We canSmelt");
			if (hasOres(player, barType)) {
				// player.sendMessage("We have ores");
				exp = getExp(barType);
				oreId = getOre(barType);
				oreId2 = getOre2(barType);
				barId = barType;
				player.smeltAmount = player.getItems().getItemAmount(getOre(barType));
				smelt(player, barType);
			} else {
				player.write(new SendMessagePacket("You do not have the required ores to smelt this."));
			...
			player.write(new SendMessagePacket("You must have a higher smithing level to smith this."));
			player.smeltInterface = false;
		    player.smeltType = 0;
		    player.smeltAmount = 0;
			player.write(new SendClearScreen());
	...
	public static void smelt(Player player, int barType) {
		if (player.smeltAmount > 0) {
			player.write(new SendClearScreen());
			if (hasOres(player, barType)) {
				player.getItems().deleteItem(oreId, player.getItems().getItemSlot(oreId), 1);
				if (oreId2 > 0)
					player.getItems().deleteItem(oreId2, player.getItems().getItemSlot(oreId2), 1);
				player.getItems().addItem(barId, 1);
				player.getSkills().addExperience(Skills.SMITHING, exp);
				player.smeltAmount--;
				player.smeltTimer = 1;
			} else {
				player.write(new SendMessagePacket("You do not have the required ores to smelt this."));
	...
	public static boolean canSmelt(Player player, int barType) {
		for (int j = 0; j < SMELT_BARS.length; j++) {
			if (barType == SMELT_BARS[j]) {
				return player.getSkills().getLevel(Skills.SMITHING) >= BAR_REQS[j];
			}
		}
		return false;
	}

	public static boolean hasOres(Player player, int barType) {
		if (getOre2(barType) > 0)
			return player.getItems().playerHasItem(getOre(barType)) && player.getItems().playerHasItem(getOre2(barType));
		else
			return player.getItems().playerHasItem(getOre(barType));
	}

	public static void readInput(Player player, int level, String type, int amounttomake) {

		if (player.getItems().getItemName(Integer.parseInt(type)).contains("Bronze")) {
			CheckBronze(player, level, amounttomake, type);
		} else {

			if (player.getItems().getItemName(Integer.parseInt(type)).contains("Iron")) {
				CheckIron(player, level, amounttomake, type);
			} else {

				if (player.getItems().getItemName(Integer.parseInt(type)).contains("Steel")) {
					CheckSteel(player, level, amounttomake, type);
				} else {

					if (player.getItems().getItemName(Integer.parseInt(type)).contains("Mith")) {
						CheckMith(player, level, amounttomake, type);
					} else {

						if (player.getItems().getItemName(Integer.parseInt(type)).contains("Adam") || player.getItems().getItemName(Integer.parseInt(type)).contains("Addy")) {
							CheckAddy(player, level, amounttomake, type);
						} else {

							if (player.getItems().getItemName(Integer.parseInt(type)).contains("Rune") || player.getItems().getItemName(Integer.parseInt(type)).contains("Runite")) {
	...
	private static void CheckIron(Player player, int level, int amounttomake, String type) {
		remove = 2351;
		...
		} else if (type.equals("1115") && level >= 33) // Platebody
		{
			...
			player.write(new SendMessagePacket("You don't have a high enough level to make this armour!"));
			player.write(new SendClearScreen());
			return;
		}

		doaction(player, item, remove, removeamount, maketimes, -1, -1, xp);

	}

	private static void CheckSteel(Player player, int level, int amounttomake, String type) {
		remove = 2353;
	...
	private static void CheckMith(Player player, int level, int amounttomake, String type) {
		...
		} else if (type.equals("1143") && level >= 53) // Med helm
		{
			...
			removeamount = 1;
			maketimes = amounttomake;
		} else if (type.equals("822") && level >= 54) // Med helm
		{
			xp = 50;
			item = 822;
			removeamount = 1;
	...
	private static void CheckRune(Player player, int level, int amounttomake, String type) {
		remove = 2363;
		...
		} else if (type.equals("1147") && level >= 88) // Med helm
		{
			...
			removeamount = 1;
			maketimes = amounttomake;
		} else if (type.equals("9144") && level >= 88) // Dart tips
		{
		...
		else if (type.equals("4824") && level >= 89) // Nails
		{
			...
			removeamount = 1;
			maketimes = amounttomake;
		} else if (type.equals("824") && level >= 89) // Arrow tips
		{
			xp = 75;
			item = 824;
			removeamount = 1;
	...
	private static void CheckAddy(Player player, int level, int amounttomake, String type) {
		remove = 2361;
		...
		else if (type.equals("823") && level >= 74) // Arrow tips
		{
			xp = 63;
			item = 823;
			removeamount = 1;
	...
	private static void CheckBronze(Player player, int level, int amounttomake, String type) {
	...
	public static void doaction(Player player, int toadd, int toremove, int toremove2, int timestomake, int NOTUSED, int NOTUSED2, int xp) {
		...
		Server.getTaskScheduler().schedule(new ScheduledTask(3) {
			...
			public void execute() {
				if (!player.isRegistered())
					this.stop();
				if (player.getItems().playerHasItem(toremove, toremove2)) {
				...
				if (maketimes > 1 && player.getItems().playerHasItem(toremove, toremove2 * 2)) {
					player.write(new SendMessagePacket("You make some " + player.getItems().getItemName(toadd) + "s."));
				} else {
					player.write(new SendMessagePacket("You make a " + player.getItems().getItemName(toadd) + "."));
				}
				if (maketimes > 0) {
					if (player.getItems().playerHasItem(toremove, toremove2)) {
						player.getItems().deleteItem2(toremove, toremove2);
						if (player.getItems().getItemName(toadd).contains("bolt") || player.getItems().getItemName(toadd).contains("dart")) {
							player.getItems().addItem(toadd, 10);
						} else if (player.getItems().getItemName(toadd).contains("arrow")) {
							player.getItems().addItem(toadd, 15);
						} else if (player.getItems().getItemName(toadd).contains("knife")) {
							player.getItems().addItem(toadd, 5);
						} else {
							player.getItems().addItem(toadd, 1);
						}
						player.getSkills().addExperience(Skills.SMITHING, xp);
					}
				}
			}

			public void onStop() {
				player.write(new SendMessagePacket("You have ran out of bars!"));
