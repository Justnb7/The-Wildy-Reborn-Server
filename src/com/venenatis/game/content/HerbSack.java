...
public class HerbSack {
	...
	public void setContents(Multiset<Integer> herbSack) {
	...
	public void handleFillSack() {
		...
		if (Arrays.stream(player.getInventory().toArray()).noneMatch(isGrimyHerb())) {
			player.getActionSender().sendMessage("There is no herbs in your inventory that can be added to the sack.");
			return;
		}
		Arrays.stream(player.getInventory().toArray()).filter(isGrimyHerb()).forEach(herb -> {
	...
	public void handleEmptySack() {
		...
		if (player.getInventory().getFreeSlots() <= 0) {
			player.getActionSender().sendMessage("You don't have enough inventory space to empty the contents of this sack.");
			return;
		}
	...
	public void handleCheckSack() {
		...
		for (int herbId : Multisets.copyHighestCountFirst(herbSack).elementSet()) {
			player.getActionSender().sendMessage(herbSack.count(herbId) + " x " + ItemDefinition.get(herbId).getName());
		}
	}
