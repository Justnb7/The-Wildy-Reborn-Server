...
public class Jewellery {
	...
	public static boolean rubItem(Player player, int slot, int itemId, boolean operating) {
		...
		if (!operating || slot == EquipmentConstants.NECKLACE_SLOT) {
			if (itemId == 1704) {
			...
			if (itemId > 1704 && itemId <= 1712) {
				...
				if (itemId % 2 == 0) { //Its an equal number..
					...
					//DialogueManager.openDialogue(player, 1712); //TODO sent dialogue
				...
				if (itemId % 2 == 0) { //Its an equal number..
					int divided = itemId / 2;
					//DialogueManager.openDialogue(player, 1718);//TODO sent dialogue
		...
		if (!operating || slot == EquipmentConstants.RING_SLOT) {
			if (itemId >= 2552 && itemId <= 2566) {
				itemId -= 2550;
				if (itemId % 2 == 0) { //Its an equal number..
					int divided = itemId / 2;
					//DialogueManager.openDialogue(player, 1722);//TODO sent dialogue
				...
				if (itemId >= 0) {
					//DialogueManager.openDialogue(player, 1353);//TODO sent dialogue
	...
	public void gemTeleport(final Player player, final Location location) {
		...
		//TODO stop any active instances
		...
		player.playGraphic(GEM_PRE_CAST_GRAPHICS);
