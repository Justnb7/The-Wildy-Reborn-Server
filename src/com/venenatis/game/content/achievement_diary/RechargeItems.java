package com.venenatis.game.content.achievement_diary;
...
public class RechargeItems {
	...
	public boolean hasAnyItem(Item... items) {
		for (Item inventory : items) {
			if (player.getInventory().contains(inventory))
				return true;
		}
		for (Item equipmentId : player.getEquipment().toArray()) {
			for (Item item : items) {
				if (equipmentId == item) {
					return true;
				}
	...
	public void replenishPrayer(int divide) {
		...
		if (player.getSkills().getLevel(Skills.PRAYER) < player.getSkills().getLevelForExperience(Skills.PRAYER)) {
			player.playAnimation(new Animation(645));
			player.getSkills().setLevel(5, (int) (player.getSkills().getLevel(5) + (player.getSkills().getLevelForExperience(5) / divide)));
			...
			player.getActionSender().sendMessage("You recharge some prayer points.");
		} else {
	...
	public void replenishRun(int amount) {
		...
		if (total > 100) {
			...
			player.getActionSender().sendString(Integer.toString(100), 149);
			player.getActionSender().sendRunEnergy();
		} else {
			player.getActionSender().sendString(Integer.toString(player.getRunEnergy()), 149);
	...
	public boolean useItem(int itemId) {
		...
		if (value <= 0) {
			player.getActionSender().sendMessage("@red@Your " + ItemDefinition.get(itemId) + " has no uses left.");
			return false;
		}
		value--;
		itemValues.put(itemId, value);
		if (itemId < 500) {
			player.getActionSender().sendMessage("@red@You now have " + value + " uses left.");
		} else {
			player.getActionSender().sendMessage("@red@Your " + ItemDefinition.get(itemId) + " now has " + value + " uses.");
		}
		return true;
	}
	...
	public void checkCharges(int itemId) {
		...
		int value = itemValues.get(itemId);
		if (value <= 0) {
			player.getActionSender().sendMessage("@blu@Your " + ItemDefinition.get(itemId) + " has no charges.");
			return;
		}
		player.getActionSender().sendMessage("@blu@Your " + ItemDefinition.get(itemId) + " has " + value + " charges left.");
	}
	...
	public void onLogin() {
		...
		for (ItemsWithRecharge item : ItemsWithRecharge.values()) {
			...
			if (!itemLastUsed.containsKey(id)) {
				itemLastUsed.put(id, convertDateToString(today));
			}
		}
	}
