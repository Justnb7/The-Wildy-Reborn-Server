...
public class PestControlRewards {
	...
	public void showInterface() {
		player.getActionSender().moveComponent(0, -100, 37100);
	...
	public boolean click(int buttonId) {
		...
		for (RewardButton button : RewardButton.values()) {
			if (button.buttonId == buttonId) {
				if (!player.getAccount().getType().isRewardSelectable(button)) {
					player.getActionSender().sendMessage("Your mode is not able to select this reward.");
					return false;
				}
				reward = button.reward;
				player.getActionSender().moveComponent(button.xOffset, button.yOffset, 37100);
	...
	public enum RewardButton {
				...
				VOID_MACE(144198, 0, 280, new ItemReward(160, new Item(8841))), 
				VOID_KNIGHT_ROBE(144201, 0, 320, new ItemReward(175, new Item(8840))), 
				VOID_MAGE_HELM(144204, 0, 360, new ItemReward(150, new Item(11663))), 
				VOID_MELEE_HELM(144207, 0, 400, new ItemReward(150, new Item(11665))), 
				VOID_KNIGHT_TOP(144210, 210, 280, new ItemReward(175, new Item(8839))), 
				VOID_KNIGHT_GLOVES(144213, 210, 320, new ItemReward(110, new Item(8842))), 
				VOID_RANGE_HELM(144216, 210, 360, new ItemReward(150, new Item(11664))), 
				FIGHTER_TORSO(144219, 210, 400, new ItemReward(300, new Item(10551))), 
				BARROWS_GLOVES(144222, 0, 440, new ItemReward(80, new Item(7462))), 
				FIGHTER_HAT(144225, 210, 440, new ItemReward(60, new Item(10548))
		);
	...
	static class ExperienceReward extends Reward {
		...
		public void purchase(Player player) {
			...
			if (player.getPestControlPoints() < cost) {
				player.getActionSender().sendMessage("You do not have the pest control points to purchase this experience.");
				return;
			}
			...
			player.getSkills().addExperience(skillId, (player.getAccount().isIron() ? experience / 26 : experience) * cost);
			player.getActionSender().sendMessage("You have received " + ((player.getAccount().isIron() ? experience / 26 : experience) * cost) + " experience in exchange for " + cost + " points.");
		}
	}
	...
	static class PackReward extends Reward {
		...
		void purchase(Player player) {
			...
			if (player.getAccount().isIron()) {
				player.getActionSender().sendMessage("Iron man are currently unable to purchase rewards from pest control.");
				return;
			}
			...
			receive.forEach(item -> player.getInventory().add(item.getId(), item.getAmount()));
		}
	}
