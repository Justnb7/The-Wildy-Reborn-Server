...
public class HelpDatabase {
	...
	public void view(Player player, int buttonId) {
		...
		if (requests.size() == 0) {
			player.getActionSender().sendMessage("There are currently no requests to view.");
			return;
		}
		if (requests.size() <= button.get().ordinal()) {
			player.getActionSender().sendMessage("There is no help request here, this is empty.");
			return;
		}
		HelpRequest request = requests.get(button.get().ordinal());
		if (Objects.isNull(request)) {
			player.getActionSender().sendMessage("The request you're trying to view cannot be found.");
			return;
		}
		player.getActionSender().sendString(WordUtils.capitalize(request.getName()) + " [" + request.getDate() + "]", 59556);
		String[] output = WordUtils.wrap(request.getMessage(), 60).split("\n");
		for (int i = 0; i < output.length; i++) {
			player.getActionSender().sendString(output[i], 59557 + i);
		}
		for (int i = output.length; i < 4; i++) {
			player.getActionSender().sendString("", 59557 + i);
		}
	}
	...
	public void delete(Player player, int buttonId) {
		...
		if (requests.size() == 0) {
			player.getActionSender().sendMessage("There are currently no requests to remove.");
			return;
		}
		if (requests.size() <= button.get().ordinal()) {
			player.getActionSender().sendMessage("There is no help request here, this is empty.");
		...
		HelpRequest request = requests.get(button.get().ordinal());
		if (Objects.isNull(request)) {
			player.getActionSender().sendMessage("The request you're trying to delete cannot be found.");
			return;
		}
		...
		if (future.after(Server.getCalendar().getInstance().getTime())) {
			player.getActionSender().sendMessage("Help requests must remain alive for 1 minute and 30 seconds before deleting.");
			return;
		}
	...
	private void updateElements(Player player) {
		...
		for (int i = 0; i < requests.size(); i++) {
			...
			player.getActionSender().sendString(WordUtils.capitalize(request.getName()) + " " + format.format(request.getDate()), 59573 + (i * 4));
			player.getActionSender().sendInterfaceConfig(0, 59571 + (i * 4));
		}
	}
	...
	public void openDatabase(Player player) {
		...
		for (int i = 0; i < 5; i++)
		player.getActionSender().sendString("", 59556 + i);
		player.getActionSender().sendInterface(59550);
	}
	...
	public boolean requestable(Player player) {
		if (requests.size() >= REQUEST_LIMIT) {
			player.getActionSender().sendMessage("We're currently experiencing a high volume of tickets right now.");
			player.getActionSender().sendMessage("You may have to submit a request at a later date.");
			return false;
		}
		if (requests.stream().anyMatch(unrequestable(player))) {
			player.getActionSender().sendMessage("You either have an open request, or someone on your IP does.");
			player.getActionSender().sendMessage("You cannot send a request until the current one is cleared.");
			return false;
		}
		return true;
	}
	...
	private Predicate<HelpRequest> unrequestable(Player player) {
		return req -> req.getName().equals(player.getUsername()) || req.getProtocol().equals(player.getHostAddress());
	}
