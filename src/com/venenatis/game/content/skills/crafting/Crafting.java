...
public enum Crafting {
	...
	public boolean itemOnItem(Player player, Item use, Item with) {
		...
		case 1:
			player.getAttributes().put(CRAFTABLE_KEY, craftable);
			player.getActionSender().sendString("\\n \\n \\n \\n \\n" + craftable.getCraftableItems()[0].getProduct().getDefinition().getName(), 2799);
			player.getActionSender().sendItemOnInterface(1746, 170, craftable.getCraftableItems()[0].getProduct().getId());
			player.getActionSender().sendChatBoxInterface(4429);
			return true;
			
		case 2:
			player.getAttributes().put(CRAFTABLE_KEY, craftable);
			player.getActionSender().sendItemOnInterface(8869, 170, craftable.getCraftableItems()[0].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8870, 170, craftable.getCraftableItems()[1].getProduct().getId());
			...
			player.getActionSender().sendChatBoxInterface(8866);
			return true;
			
		case 3:
			player.getAttributes().put(CRAFTABLE_KEY, craftable);
			player.getActionSender().sendItemOnInterface(8883, 170, craftable.getCraftableItems()[0].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8884, 170, craftable.getCraftableItems()[1].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8885, 170, craftable.getCraftableItems()[2].getProduct().getId());
			...
			player.getActionSender().sendChatBoxInterface(8880);
			return true;
			
		case 4:
			player.getAttributes().put(CRAFTABLE_KEY, craftable);
			player.getActionSender().sendItemOnInterface(8902, 170, craftable.getCraftableItems()[0].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8903, 170, craftable.getCraftableItems()[1].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8904, 170, craftable.getCraftableItems()[2].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8905, 170, craftable.getCraftableItems()[3].getProduct().getId());
			...
			player.getActionSender().sendChatBoxInterface(8899);
			return true;
			
		case 5:
			player.getAttributes().put(CRAFTABLE_KEY, craftable);
			player.getActionSender().sendItemOnInterface(8941, 170, craftable.getCraftableItems()[0].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8942, 170, craftable.getCraftableItems()[1].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8943, 170, craftable.getCraftableItems()[2].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8944, 170, craftable.getCraftableItems()[3].getProduct().getId());
			player.getActionSender().sendItemOnInterface(8945, 170, craftable.getCraftableItems()[4].getProduct().getId());
			player.getActionSender().sendString("\\n \\n \\n \\n \\n".concat("Body"), 8949);
			player.getActionSender().sendString("\\n \\n \\n \\n \\n".concat("Chaps"), 8953);
			player.getActionSender().sendString("\\n \\n \\n \\n \\n".concat("Vambraces"), 8957);
			player.getActionSender().sendString("\\n \\n \\n \\n \\n".concat("Bandana"), 8961);
			player.getActionSender().sendString("\\n \\n \\n \\n \\n".concat("Boots"), 8965);
			player.getActionSender().sendChatBoxInterface(8938);
			return true;
	...
	public boolean clickButton(Player player, int button) {
		...
		/** Option 1 - Make all */
		case 6211:
			start(player, craftable, 0, player.getInventory().getAmount(craftable.getWith().getId()));
			return true;
	...
	public boolean start(Player player, Craftable craftable, int index, int amount) {
		...
		if (player.getSkills().getLevel(Skills.CRAFTING) < item.getLevel()) {
			SimpleDialogues.sendStatement(player, "<col=369>You need a Crafting level of " + item.getLevel() + " to do that.");
			return true;
		}
		...
		if (!player.getInventory().hasAllItems(craftable.getIngediants(index))) {
			...
			player.getActionSender().sendMessage("You need " + requiredItem.getAmount() + " piece" + (requiredItem.getAmount() > 1 ? "s" : "") + " of " + requiredItem.getDefinition().getName().toLowerCase() + " to make" + productAmount + " " + product.getDefinition().getName().toLowerCase() + ".");
			return true;
		}
		World.getWorld().schedule(new Task(player, 2, true, StackType.NEVER_STACK, BreakType.ON_MOVE) {
			private int iterations = 0;

			@Override
			public void execute() {

				player.playAnimation(new Animation(craftable.getAnimation()));
				player.getSkills().addExperience(Skills.CRAFTING, item.getExperience());
				player.getInventory().remove(craftable.getIngediants(index));
				player.getInventory().add(item.getProduct());

				if (craftable.getProductionMessage() != null) {
					player.getActionSender().sendMessage(craftable.getProductionMessage());
				}
