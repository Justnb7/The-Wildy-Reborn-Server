...
public class Allotments {
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (allotmentData.getLevelRequired() > player.getSkills().getLevel(Skills.FARMING)) {
			SimpleDialogues.sendStatement(player, "You need a farming level of " + allotmentData.getLevelRequired() + " to plant this seed.");
			return true;
		}
		...
		if (!player.getInventory().hasItemAmount(allotmentData.getSeedId(), allotmentData.getSeedAmount())) {
			SimpleDialogues.sendStatement(player, "You need atleast " + allotmentData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
		player.playAnimation(new Animation(FarmingConstants.SEED_DIBBING));
		allotmentStages[allotmentFieldsData.getAllotmentIndex()] = 4;
		player.getInventory().removeFromSlot(player.getInventory().searchSlot(seedId), seedId, allotmentData.getSeedAmount());
		...
		World.getWorld().schedule(new Task(player, 2, false, StackType.NEVER_STACK, BreakType.NEVER) {
			...
			public void execute() {
				...
				if (allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] == 0) {
					allotmentHarvest[allotmentFieldsData.getAllotmentIndex()] = (int) (1 + (START_HARVEST_AMOUNT + Utility.random((END_HARVEST_AMOUNT + (player.getEquipment().contains(7409) ? 15 : 0)) - START_HARVEST_AMOUNT)) * (1));
				}
				...
				player.getSkills().addExperience(Skills.FARMING, allotmentData.getHarvestXp());
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] == 0) {
			SimpleDialogues.sendStatement(player, "This is an allotment patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] == 3) {
			SimpleDialogues.sendStatement(player, "This is an allotment patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			World.getWorld().schedule(new Task(player, 5, false, StackType.NEVER_STACK, BreakType.NEVER) {
				...
				public void execute() {
					if (allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4 < inspectData.getMessages().length - 2) {
						SimpleDialogues.sendStatement(player, inspectData.getMessages()[allotmentStages[allotmentFieldsData.getAllotmentIndex()] - 4]);
	...
	public void handleFlowerProtection(int index) {
		...
		if (!allotmentWatched[index]) {// Misc.random(100) <= maxChance) {
			...
			if (player.getFarming().getFlowers().flowerState[indexGiven] != 3 && player.getFarming().getFlowers().hasFullyGrown[indexGiven] && player.getFarming().getFlowers().flowerSeeds[indexGiven] == allotmentData.getFlowerProtect()) {
				...
				player.getActionSender().sendMessage("One of your crops is diseased!");
			}
		}
