...
public class Compost {
	...
	public void updateCompostBin(int index) {
		...
		Server.getGlobalObjects().add(new GameObject(finalObject, x, y, player.getZ(), 10, CompostBinLocations.forId(index).getObjectFace()));
	}
	...
	public void retrieveCompost(final int index) {
		...
		World.getWorld().schedule(new Task(player, 2, false, StackType.NEVER_STACK, BreakType.NEVER) {
			...
			public void execute() {
				...
				player.getSkills().addExperience(Skills.FARMING, finalItem == COMPOST ? COMPOST_EXP_RETRIEVE : finalItem == SUPER_COMPOST ? SUPER_COMPOST_EXP_RETRIEVE : ROTTEN_TOMATOES_EXP_RETRIEVE);
