...
public class SlayerTaskManagement {
	...
	public static boolean resetTask(Player player) {
		...
		if (player.getSlayerPoints() >= 10) {
			...
			player.getActionSender().sendMessage(
					"Your slayer task and streak has been reset , talk to any slayer master for a new one.");
			return true;
		} else {
	...
	public static Task turaelTask(Player player) {
		...
		for (int i = 0; i < array.size(); i++) {
			Turael t = array.get(i);
			max += t.getPercentage();
		}
		max = Math.min(100, max); // choose from lowest value out of (100, cap)

		RandomGenerator r = new RandomGenerator();
		// The RNG value / index in the possible tasks array we're gonna take.
		int random = r.exclusive(1, max);
		...
		// Mark the start of this block as 'main'
		main: for (int i = 0; i < array.size(); i++) {
			Turael t = array.get(i);
			currentCount += t.getPercentage();
		...
		if (task == null) {
			System.err.println("wtf lol " + random + "/" + array.size());
		}
		return task;
	}
	...
	public static int assignTaskAmount(Player player, int task, int difficulty) {
		int amount = difficulty == 0 ? 13 + Utility.random(5, 10)
				: difficulty == 1 ? 13 + Utility.random(5, 10)
						: difficulty == 3 ? 25 + Utility.random(15, 35)
		...
		for (Entry<Integer, Integer> entrys : player.getSlayerInterface().getExtensions().entrySet()) {
			if (entrys.getValue() == 270) {
				if (task == 270 || task == 272 || task == 274) { // dragon
																	// extensions
	...
	public static Task duradelTask(Player player) {
		Task task = null;
		...
		main: for (int i = 0; i < array.size(); i++) {
			...
			// Hit in the table
			int lowbound = (currentCount - t.getPercentage());
			// System.out.println("GOAL: " + random + " (max possibility: " +
			// max + ") ... trying " + lowbound + " -> " + random + " <- " +
			// currentCount);
			if (currentCount >= random && lowbound <= random) {
	...
	public static void decreaseTask(Player player, NPC npc) {
		...
		if (player != null) {
			...
			// The player has completed their task, we can go ahead and reward
			// them.
			if (player.getSlayerTaskAmount() <= 0) {
				...
				if (player.getSlayerTaskDifficulty() == 0) {
					player.getActionSender()
							.sendMessage("You have completed your slayer assignment. You don't gain any slayer");
					player.getActionSender()
							.sendMessage("points for completing a task with Turael. Go back and speak to Turael");
					...
					if (Constants.SLAYER_REWARDS) {
						player.getActionSender().sendMessage(
								"You have completed your slayer assignment. Double Slayer points is active");
						player.getActionSender()
								.sendMessage("meaining you get @blu@"
										+ (Mazchna.getStreak(player) > 0 ? Mazchna.getStreak(player) : 4)
										+ "@bla@ Slayer Points. Please speak to a Slayer Master");
						...
						player.getActionSender()
								.sendMessage("You have completed your slayer assignment. You gain @blu@ "
										+ (Mazchna.getStreak(player) > 0 ? Mazchna.getStreak(player) : 2)
										+ "@bla@ Slayer Points!");
						player.getActionSender()
								.sendMessage("Please speak to a Slayer Master to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
								+ (Mazchna.getStreak(player) > 0 ? Mazchna.getStreak(player) : 2));
						if (Mazchna.getStreak(player) > 0)
							player.getActionSender().sendMessage("You hit your streak of: " + player.getSlayerStreak()
					...
					if (Constants.SLAYER_REWARDS) {
						...
						player.getActionSender()
								.sendMessage("meaining you get @blu@"
										+ (Vannaka.getStreak(player) > 0 ? Vannaka.getStreak(player) : 8)
										+ "@bla@ Slayer Points. Please speak to a Slayer Master");
						player.getActionSender().sendMessage("to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
								+ (Vannaka.getStreak(player) > 0 ? Vannaka.getStreak(player) : 8));
						...
						player.getActionSender()
								.sendMessage("You have completed your slayer assignment. You gain @blu@"
										+ (Vannaka.getStreak(player) > 0 ? Vannaka.getStreak(player) : 4)
										+ "@bla@ Slayer Points!");
						player.getActionSender()
								.sendMessage("Please speak to a Slayer Master to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
					...
					if (Constants.SLAYER_REWARDS) {
						...
						player.getActionSender()
								.sendMessage("meaining you get @blu@"
										+ (Chaeldar.getStreak(player) > 0 ? Chaeldar.getStreak(player) : 20)
										+ "@bla@ Slayer Points. Please speak to a Slayer Master");
						player.getActionSender().sendMessage("to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
								+ (Chaeldar.getStreak(player) > 0 ? Chaeldar.getStreak(player) : 20));
						...
						player.getActionSender()
								.sendMessage("You have completed your slayer assignment. You gain @blu@"
										+ (Chaeldar.getStreak(player) > 0 ? Chaeldar.getStreak(player) : 10)
										+ "@bla@ Slayer Points!");
						player.getActionSender()
								.sendMessage("Please speak to a Slayer Master to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
					...
					if (Constants.SLAYER_REWARDS) {
						...
						player.getActionSender()
								.sendMessage("meaining you get @blu@"
										+ (Nieve.getStreak(player) > 0 ? Nieve.getStreak(player) : 24)
										+ "@bla@ Slayer Points. Please speak to a Slayer Master");
						player.getActionSender().sendMessage("to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
								+ (Nieve.getStreak(player) > 0 ? Nieve.getStreak(player) : 24));
						...
						player.getActionSender()
								.sendMessage("You have completed your slayer assignment. You gain @blu@"
										+ (Nieve.getStreak(player) > 0 ? Nieve.getStreak(player) : 12)
										+ "@bla@ Slayer Points!");
						player.getActionSender()
								.sendMessage("Please speak to a Slayer Master to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
					...
					if (Constants.SLAYER_REWARDS) {
						...
						player.getActionSender()
								.sendMessage("meaining you get @blu@"
										+ (Duradel.getStreak(player) > 0 ? Duradel.getStreak(player) : 30)
										+ "@bla@ Slayer Points. Please speak to a Slayer Master");
						player.getActionSender().sendMessage("to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
								+ (Duradel.getStreak(player) > 0 ? Duradel.getStreak(player) : 30));
						...
						player.getActionSender()
								.sendMessage("You have completed your slayer assignment. You gain @blu@"
										+ (Duradel.getStreak(player) > 0 ? Duradel.getStreak(player) : 15)
										+ "@bla@ Slayer Points!");
						player.getActionSender()
								.sendMessage("Please speak to a Slayer Master to retrieve an another assignment.");
						player.setSlayerPoints(player.getSlayerPoints()
				...
				if (player.getSlayerStreak() % 1000 == 0) {
					PlayerUpdating.executeGlobalMessage(
							"<shad=000000><col=FF5E00>News: " + Utility.formatName(player.getUsername())
									+ " has just completed " + player.getSlayerStreak() + "x Slayer tasks in a row!");
	...
	public enum Teleports {
		 ...
		 * Turael
		 */
		BANSHEE(414, "Banshee", new Location(3441, 3565, 0)), BAT(2834, "Bat", new Location(2912, 9837, 0)), BIRDS(2692, "Chicken", new Location(3232, 3299, 0)), BEAR(3423, "Bear", new Location(2712, 3335, 0)), CAVE_BUGS(481, "Cave bug", new Location(3160, 9573, 0)), CAVE_CRAWLERS(406, "Cave crawler", new Location(3193, 9577, 0)), CAVE_SLIMES(480, "Cave slime", new Location(3158, 9592, 0)), COWS(2805, "Cow", new Location(3260, 3278, 0)), CRAWLING_HANDS(448, "Crawling hand", new Location(3411, 3537, 0)), DWARVES(291, "Dwarve", new Location(2918, 9758, 0)), GHOSTS(85, "Ghost", new Location(3277, 3658, 0)), GOBLINS(2245, "Goblin", new Location(2572, 3388, 0)), KALPHITES(955, "Kalphite", new Location(3760, 5854, 0)),

		/**
		 * Chaeldar
		 */
		ABYSSAL_DEMON(415, "Abyssal demon", new Location()), ABERRANT_SPECTRE(3, "Aberrant spectre", new Location(2393, 9780, 0)), BLACK_DEMON(1432, "Black demon", new Location(2701, 9490, 0)), HELLHOUND(135, "Hellhound", new Location(2407, 9785, 0)), CAVE_KRAKEN(492, "Cave kraken", new Location(2481, 9799, 0)), STEEL_DRAGON(274, "Steel dragon", new Location(2312, 9775, 0)), IRON_DRAGON(272, "Iron dragon", new Location(2326, 9736, 0)), ANKOU(2514, "Ankou", new Location(2376, 9758, 0)), BASILISK(417, "Basilisk", new Location(2742, 10002, 0)), BLUE_DRAGON(268, "Blue dragon", new Location(2894, 9794, 0)), BLOODVELD(484, "Bloodveld", new Location(2436, 9794, 0)), BRONZE_DRAGON(270, "Bronze dragon", new Location(2349, 9742, 0)), COCKATRICE(419, "Cockatrice", new Location(2799, 10030, 0)), FIRE_GIANT(2075, "Fire giant", new Location(2352, 9754, 0)), GARGOYLE(412, "Gargoyle", new Location(3434, 3539, 2)), GREEN_DRAGON(264, "Green dragon", new Location(2973, 3620, 0)), HILL_GIANT(2098, "Hill giant", new Location(3298, 3650, 0)), ICE_WARRIOR(2841, "Ice warrior", new Location(3036, 9541, 0)), INFERNAL_MAGE(443, "Infernal mage", new Location(3438, 3561, 0)), JELLY(437, "Jelly", new Location(2705, 10030, 0)), KURASK(410, "Kurask", new Location(2702, 9992, 0)), PYREFIEND(433, "Pyrefiend", new Location(2758, 10011, 0)), NECHRYAEL(11, "Nechryael", new Location(2427, 9752, 0)), TUROTH(432, "Turoth", new Location(2717, 10012, 0)), DUST_DEVIL(423, "Dust devil", new Location(3425, 3551, 1)), LESSER_DEMON(2005, "Lesser demon", new Location(2932, 9809, 0)), BLACK_DRAGON(252, "Black dragon", new Location(2331, 9747, 0)),
		 ...
		 * Nieve
		 */
		DARK_BEAST(4005, "Dark beast", new Location(3421, 3566, 2)), CAVE_HORROR(3209, "Cave horror", new Location(2802, 10015, 0)), GREATER_DEMON(2026, "Greater demon", new Location(2358, 9735, 0)),

		/**
		 * Duradel
		 */
		KING_BLACK_DRAGON(239, "King black dragon", new Location(2997, 3849, 0)), CHAOS_ELEMENTAL(2054, "Chaos elemental", new Location(3284, 3913, 0)), DAGANNOTH_SUPREME(2265, "Dagannoth supreme", new Location(1907, 4365, 0)), DAGGANOTH_REX(2267, "Dagannoth rex", new Location(1907, 4365, 0)), DAGGANOTH_PRIME(2266, "Dagannoth prime", new Location(1907, 4365, 0)), VENENATIS(6610, "Venenatis", new Location(3334, 3741, 0)), VETION(6611, "Vet'ion", new Location(3210, 3780, 0)), SCORPIA(6615, "Scorpia", new Location(3233, 3943, 0)), CALLISTO(6609, "Callisto", new Location(2271, 4681, 0)), KRIL_TSUTSAROTH(3129, "K'ril tsutsaroth", new Location(2924, 5340, 2)), KREE_ARRA(3162, "Kree'arra", new Location(2840, 5289, 2)), COMMANDER_ZILYANA(2205, "Commander zilyana", new Location(2912, 5266, 0)), GENERAL_GRAARDOR(2215, "General graardor", new Location(2857, 5354, 2)), KRAKEN(494, "Kraken", new Location(2481, 9799, 0)), BARRELCHEST(6342, "Barrelchest", new Location(3277, 3882, 0)), ZOMBIES_CHAMPION(3359, "Zombies champion", new Location(3020, 3631, 0));
