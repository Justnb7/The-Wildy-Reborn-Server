...
public final class TeleportationInterface {
	...
	private static boolean teleport(Player player, int buttonId) {
		...
		if(player.teleportType == null || player.teleportData == null) {
			player.getActionSender().sendMessage("You haven't specified where you want to teleport yet.");
			return true;
		}

		TeleportData data = player.teleportData;
		player.getTeleportAction().teleport(new Location(data.x, data.y, data.height), TeleportTypes.SPELL_BOOK, false);
	...
	public static boolean selectTeleport(Player player, int buttonId) {
		...
		if(player.teleportType == null) {
			player.getActionSender().sendMessage("Please select a menu before selecting a teleport.");
			return true;
		}
		...
		if(player.teleportType.data.size() - 1 < index || player.teleportType.data.get(index) == null) {
			player.getActionSender().sendMessage("There is no teleport on this spot.");
			return true;
		}
		
		TeleportData data = player.teleportType.data.get(index);
		
		player.getActionSender().sendString(data.information, 31007);
		
		IntStream.range(31011, 31049).forEach(id -> player.getActionSender().sendChangeSprite(id, (byte) 0));
		player.getActionSender().sendChangeSprite(LINE_BUTTON_IDS_FOR_LINE_IDS.get(buttonId) - 1, (byte) 1);
	...
	public static boolean open(Player player) {
		...
		player.lastTeleport = System.currentTimeMillis();
		IntStream.range(0, 12).forEach(id -> player.getActionSender().sendString("", LINE_IDS.get(id)));
		IntStream.range(18011, 18069).forEach(id -> player.getActionSender().sendChangeSprite(id, (byte) 0));

		player.getActionSender().sendString("By clicking teleport below, you are aware that you might \\n have chosen a location which may be placed within \\n the wilderness. And is aware of the risks that follows.", 18007);

		selectMenu(player, player.lastMenuChosen == 0 ? 70132 : player.lastMenuChosen);

		IntStream.rangeClosed(0, 6).forEach(id -> {
			if (TeleportType.valueOf(id).isPresent())
				player.getActionSender().sendString(TeleportType.valueOf(id).get().toString(), MENU_LINE_IDS.get(id));
		});
		player.getActionSender().sendInterface(18000);
		return true;
	}
	...
	public static boolean selectMenu(Player player, int buttonId) {
		...
		IntStream.range(18011, 18069).forEach(id -> player.getActionSender().sendChangeSprite(id, (byte) 0));
		player.getActionSender().sendChangeSprite(MENU_BUTTON_IDS_FOR_MENU_LINE_IDS.get(buttonId) - 1, (byte) 1);
		
		IntStream.rangeClosed(0, 12).forEach(id -> {
			if(TeleportType.valueOf(type, id).isPresent()) {
				player.getActionSender().sendString(TeleportType.valueOf(type, id).get().toString(), LINE_IDS.get(id));
			}
		});
		return true;
	}

	private static final ImmutableList<Integer> MENU_LINE_IDS = ImmutableList.of(18053, 18056, 18059, 18062, 18065, 18068);

	private static final ImmutableList<Integer> LINE_IDS = ImmutableList.of(18013, 18016, 18019, 18022, 18025, 18028, 18033, 18036, 18039, 18042, 18045, 18048);
	
	private static final ImmutableList<Integer> LINE_BUTTON_IDS = ImmutableList.of(70092, 70095, 70098, 70101, 70104, 70107, 70112, 70115, 70118, 70121, 70124, 70127); 
	
	private static final ImmutableMap<Integer, Integer> LINE_BUTTON_IDS_FOR_LINE_IDS = ImmutableMap.copyOf(Utility.map(LINE_BUTTON_IDS, LINE_IDS));
	
	private static final ImmutableMap<Integer, Integer> MENU_BUTTON_IDS_FOR_MENU_LINE_IDS = ImmutableMap.copyOf(Utility.map(ImmutableList.of(70132, 70135, 70138, 70141, 70143, 70147), MENU_LINE_IDS));
	
	public enum TeleportType {
		TRAINING(70132, TeleportData.CHICKENS, TeleportData.ROCK_CRABS, TeleportData.COWS),
		MONSTERS(70135, TeleportData.SLAYER_TOWER, TeleportData.EDGEVILLE_DUNGEON, TeleportData.TAVERLY_DUNGEON, TeleportData.BRIMHAVEN_DUNGEON, TeleportData.RELLEKKA_DUNGEON, TeleportData.STRONGHOLD_CAVE, TeleportData.MITHRIL_DRAGONS, TeleportData.LLETYA, TeleportData.DEMONIC_GORILLAS, TeleportData.ICE_DUNGEON, TeleportData.DAGANNOTH_CAVE),
		BOSSES(70138, TeleportData.KING_BLACK_DRAGON, TeleportData.CHAOS_ELEMENTAL, TeleportData.GODWARS, TeleportData.BARRELCHEST, TeleportData.KRAKEN, TeleportData.VENENATIS, TeleportData.VETION, TeleportData.CALLISTO, TeleportData.GIANT_MOLE, TeleportData.LIZARDMAN_CANYON, TeleportData.ABYSSAL_SIRE, TeleportData.RAIDS),
		MINIGAMES(70141, TeleportData.PEST_CONTROL, TeleportData.DUEL_ARENA, TeleportData.FIGHT_CAVES, TeleportData.BARROWS, TeleportData.WARRIORS_GUILD, TeleportData.MAGE_ARENA, TeleportData.LIGHTHOUSE, TeleportData.RECIPE_FOR_DISASTER),
		SKILLING(70144, TeleportData.SKILLING_AREA, TeleportData.HUNTING_GROUNDS, TeleportData.WOODCUTTING_GUILD),
		PLAYER_KILLING(70147, TeleportData.WILDERNESS_PORTALS, TeleportData.WEST_DRAGONS, TeleportData.EAST_DRAGONS, TeleportData.HILL_GIANTS);
	...
	public enum TeleportData {
		 ...
		 * Training
		 */
		CHICKENS(70092, 3236, 3295, 0, "Suitable training ground for beginners"),
		ROCK_CRABS(70095, 2679, 3717, 0, "Suitable training ground for beginners"),
		COWS(70098, 3253, 3267, 0, "Suitable training ground for beginners"),
