...
public class BoundaryManager {
	 ...
	 * Logger instance.
	 */
	private static final Logger logger = Logger.getLogger(BoundaryManager.class.getName());
	...
	public static void init() throws IOException {
		...
		if (file.exists()) {
			...
			logger.info("Loaded " + boundaries.size() + " boundary definitions.");
		} else {
	...
	public static boolean isWithinBoundaryNoZ(Location location, String name) {
		for (Boundary boundary : boundaryForName(name)) {
			if (location.getX() >= boundary.getBottomLeft().getX() && location.getX() <= boundary.getTopRight().getX()
					&& location.getY() >= boundary.getBottomLeft().getY()
					&& location.getY() <= boundary.getTopRight().getY()) {
				return true;
			}
