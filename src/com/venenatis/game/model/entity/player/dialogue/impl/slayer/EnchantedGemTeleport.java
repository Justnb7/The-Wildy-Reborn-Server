...
public class EnchantedGemTeleport extends Dialogue {
	...
	protected void select(int index) {
		if (getPhase() == 0) {
			if (index == 1) {
				player.getTeleportAction().teleport(new Location(3507, 3503, 0), TeleportTypes.MODERN, true);
				player.getTeleportAction().teleport(new Location(3507, 3503, 0), TeleportTypes.MODERN, true);
				player.getActionSender().sendMessage("You teleport to Mazchna's location.");
			} else if (index == 2) {
				player.getTeleportAction().teleport(new Location(3147, 9913, 0), TeleportTypes.MODERN, true);
				player.getActionSender().sendMessage("You teleport to Vannaka's location.");
			} else if (index == 3) {
				player.getTeleportAction().teleport(new Location(3079, 3504, 0), TeleportTypes.MODERN, true);
				player.getActionSender().sendMessage("You teleport to Chaeldar's location.");
			} else if (index == 4) {
				player.getTeleportAction().teleport(new Location(2430, 3417, 0), TeleportTypes.MODERN, true);
				player.getActionSender().sendMessage("You teleport to Nieve's location.");
			} else if (index == 5) {
				player.getTeleportAction().teleport(new Location(2928, 3536, 0), TeleportTypes.MODERN, true);
