...
public class TuraelDialogue extends Dialogue {
	...
	protected void start(Object... parameters) {
		send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "'Ello, and what are you after, then?");
	...
	protected void next() {
		...
		if (getPhase() == 0) {
			send(DialogueType.CHOICE, DEFAULT_OPTION_TITLE, "Who are you?", "Do you have anything for trade?",
					"Erm... nothing...");
			setPhase(1);
		} else {
			if (getPhase() == 2) {
				send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "I'm one of the Slayer Masters.",
						"I can offer you an assignment where you can kill",
						"an assigned monster for 'Slayer' experience.");
				setPhase(3);
			} else {
				if (getPhase() == 3) {
					send(DialogueType.NPC, NPC_ID, Expression.DEFAULT,
					...
					if (getPhase() == 4) {
						send(DialogueType.PLAYER, Expression.DEFAULT, "Wait... there's more Slayer Masters?");
						setPhase(5);
					} else {
						if (getPhase() == 5) {
							send(DialogueType.NPC, NPC_ID, Expression.HAPPY, "Yes!",
							...
							if (getPhase() == 6) {
								...
								if (getPhase() == 7) {
									...
									if (getPhase() == 8) {
										send(DialogueType.STATEMENT, "Turael hands you an @blu@Enchanted gem@bla@.");
										...
										if (getPhase() == 9) {
											...
											if (getPhase() == 10) {
												...
												if (getPhase() == 11) {
													send(DialogueType.CHOICE, DEFAULT_OPTION_TITLE, "Got any tips for me?", "Okay great!");
													setPhase(12);
												} else {
													if (getPhase() == 13) {
														send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "Slayer is an independent skill.", "Slayer Monsters are typically found in dungeons but", "they can be scattered amongst Divine-Reality.");
														setPhase(14);
													} else {
														if (getPhase() == 14) {
															send(DialogueType.PLAYER, Expression.DEFAULT, "Okay, great!");
															player.setFirstSlayerTask(true);
															setPhase(16);
														} else {
															if (getPhase() == 15) {
																send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "Yes, but you have yet to start the 'Slayer' skill.");
																...
																if (getPhase() == 16) {
																	...
																	if (getPhase() == 17) {
																		if (Slayer.hasTask(player)) {
																			...
																			send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "Someone of your strength should go and see Vannaka.", "Would you like to get an assignment from him?");
																			setPhase(19);
																		...
																		if (getPhase() == 19) {
																			...
																			if (getPhase() == 21) {
																				if (Slayer.suitableMaster(player) == SlayerMasters.MAZCHNA) {
																					send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "Okay. Mazchna can be found in Canifis.", "You can get there using the 'Teleport' option", "on an Enchanted gem.");
																					setPhase(16);
																				} else if (Slayer.suitableMaster(player) == SlayerMasters.VANNAKA) {
																					send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "Okay. Vannaka can be found in Edgeville Dungeon.", "You can get there using the 'Teleport' option", "on an Enchanted gem.");
																					setPhase(16);
																				} else if (Slayer.suitableMaster(player) == SlayerMasters.CHAELDAR) {
																					send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "Okay. Chaeldar can be found in Zanaris.", "You can get there using the 'Teleport' option", "on an Enchanted gem.");
																					setPhase(16);
																				...
																				if (getPhase() == 22) {
																					...
																					if (getPhase() == 23) {
																						...
																						if (getPhase() == 24) {
																							send(DialogueType.CHOICE, DEFAULT_OPTION_TITLE, "I'd like an assignment.", "Do you have anything for trade?", "Nevermind.");
																							setPhase(25);
																						} else {
																							if (getPhase() == 26) {
																								if (Slayer.hasTask(player)) {
																									send(DialogueType.NPC, NPC_ID, Expression.DEFAULT, "You already have an assignment.");
																								} else {
																									setPhase(27);
																								}
																							} else {
	...
	protected void select(int index) {
		...
		if (getPhase() == 1) {
			if (index == 1) {
				...
				send(DialogueType.PLAYER, Expression.DEFAULT, "Erm... nothing...");
				setPhase(16);
			}
		} else {
			if (getPhase() == 12) {
				if (index == 1) {
					send(DialogueType.PLAYER, Expression.DEFAULT, "Got any tips for me?");
					setPhase(13);
				} else if (index == 2) {
					send(DialogueType.PLAYER, Expression.DEFAULT, "Okay great!");
					setPhase(16);
				...
				if (getPhase() == 20) {
					...
					if (getPhase() == 25) {
						if (index == 1) {
							send(DialogueType.PLAYER, Expression.DEFAULT, "I'd like an assignment.");
							setPhase(17);
						} else if (index == 2) {
