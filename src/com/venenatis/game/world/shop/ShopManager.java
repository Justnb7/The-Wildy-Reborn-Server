...
public class ShopManager {
	...
	public static void buy(Player player, int id, int amount, int slot) {
		...
		if (removed == 0 && price > 0) {
			...
			if (shop.getCurrency() != Currency.GEAR_POINTS && shop.getCurrency() != Currency.DONATOR_TICKETS && shop.getCurrency() != Currency.BOUNTIES) {
			...
			player.getActionSender().sendItemOnInterface(3823, player.getInventory().toArray());
			Achievements.activate(player, Achievement.SPENDER, price);
		}
	}
